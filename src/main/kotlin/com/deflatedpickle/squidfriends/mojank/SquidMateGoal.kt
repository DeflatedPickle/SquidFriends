...
class SquidMateGoal(
    ...
    override fun canStart(): Boolean {
        return if (!(animal as IBreedable).isInLove()) {
            false
        } else {
            mate = findMate()
            mate != null
    ...
    override fun shouldContinue(): Boolean {
        return mate!!.isAlive && (mate as IBreedable).isInLove() && timer < 60
    }
    ...
    private fun findMate(): SquidEntity? {
        ...
        for (animalEntity2 in list) {
            if ((animal as IBreedable).canBreedWith(animalEntity2) && animal.squaredDistanceTo(animalEntity2) < d) {
    ...
    private fun breed() {
        (animal as IBreedable).breed(world as ServerWorld, mate)
    }
}
